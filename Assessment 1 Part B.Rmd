...
fastfood%>%
  ...
  mutate(total_fat=max(total_fat, na.rm=TRUE))%>%
...
fastfood%>%
  group_by(restaurant, cholesterol)%>%
  mutate(cholesterol=min(cholesterol, na.rm=TRUE))%>%
...
#I believe this data viz is a simple way to differentiate between the various restaurants on the x axis whilst also able to view on the y axis the total_fat of these restaurants.
